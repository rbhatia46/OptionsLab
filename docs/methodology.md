...
4. Apply selection, IV/return filters, estimated net credit, and collateral reserve using only this past information.
...
Defined-width condors/spreads reserve the maximum wing width multiplied by base BTC quantity, without offsetting estimated premium. Other structures reserve configured notional margin for every short leg. Reserve must fit the chosen fraction of current realized equity. Long-leg offsets, changing collateral requirements, cross-margin, liquidation, and interim unhedged margin while legs fill are not modelled.
...
Holdout P&L retains the original replay's sizing and entry decisions, then rebases reporting to initial capital. Prior development losses can therefore affect holdout capital checks. This is a sequential strategy continuation, not an independently reset account or a walk-forward retraining exercise.
